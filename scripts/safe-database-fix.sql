-- =====================================================
-- SAFE DATABASE FIX FOR PATHNITI
-- This version preserves existing data
-- =====================================================

-- 1. FIRST, BACKUP AND UPDATE EXISTING DATA
-- Update any existing data to use correct role values
UPDATE public.profiles 
SET role = 'student' 
WHERE role = 'counselor' OR role = 'college_admin';

-- 2. CREATE A TEMPORARY COLUMN WITH CORRECT VALUES
ALTER TABLE public.profiles 
ADD COLUMN role_temp TEXT;

-- 3. COPY DATA TO TEMPORARY COLUMN
UPDATE public.profiles 
SET role_temp = CASE 
    WHEN role = 'student' THEN 'student'
    WHEN role = 'admin' THEN 'admin'
    WHEN role = 'counselor' THEN 'student'  -- Convert to student
    WHEN role = 'college_admin' THEN 'student'  -- Convert to student
    ELSE 'student'
END;

-- 4. DROP THE OLD ENUM TYPE (this will also drop the role column)
DROP TYPE IF EXISTS user_role CASCADE;

-- 5. CREATE THE NEW ENUM TYPE WITH CORRECT VALUES
CREATE TYPE user_role AS ENUM ('student', 'admin', 'college');

-- 6. ADD THE ROLE COLUMN BACK WITH CORRECT TYPE
ALTER TABLE public.profiles 
ADD COLUMN role user_role DEFAULT 'student';

-- 7. COPY DATA FROM TEMPORARY COLUMN TO NEW ROLE COLUMN
UPDATE public.profiles 
SET role = role_temp::user_role;

-- 8. DROP THE TEMPORARY COLUMN
ALTER TABLE public.profiles 
DROP COLUMN role_temp;

-- 9. CREATE MISSING COLLEGE_PROFILES TABLE
...
-- 10. ENABLE RLS ON COLLEGE_PROFILES
ALTER TABLE public.college_profiles ENABLE ROW LEVEL SECURITY;

-- 11. CREATE MISSING RLS POLICIES FOR PROFILES
...
-- 12. CREATE RLS POLICIES FOR COLLEGE_PROFILES
...
-- 14. CREATE TRIGGER FOR COLLEGE_PROFILES
...
SELECT 
    'VERIFICATION' as category,
    'Existing Profiles Data' as item,
    COUNT(*)::text as values,
    CASE 
        WHEN COUNT(*) > 0
        THEN '✅ DATA PRESERVED'
