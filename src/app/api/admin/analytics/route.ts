...
export async function GET(_request: NextRequest) {
  try {
    ...
    const students = userStats.data?.filter(u => u.role === 'student').length || 0;
    const colleges = userStats.data?.filter(u => u.role === 'college').length || 0;
    const admins = userStats.data?.filter(u => u.role === 'admin').length || 0;

    // Calculate new users this week
    const weekAgo = new Date();
    weekAgo.setDate(weekAgo.getDate() - 7);
    const newUsersThisWeek = userStats.data?.filter(user => 
      new Date(user.created_at) > weekAgo
    ).length || 0;

    // Calculate college statistics
    const totalColleges = collegeStats.data?.length || 0;
    const governmentColleges = collegeStats.data?.filter(c => c.type === 'government').length || 0;
    const privateColleges = collegeStats.data?.filter(c => c.type === 'private').length || 0;

    // Calculate assessment statistics
    const totalAssessments = assessmentStats.data?.length || 0;
    let averageScore = 0;
    if (assessmentStats.data && assessmentStats.data.length > 0) {
      const scores = assessmentStats.data.map(assessment => {
        if (assessment.aptitude_scores) {
          const scoreValues = Object.values(assessment.aptitude_scores) as number[];
    ...
    // Get recent activity
    const recentActivityList = recentActivity.data?.map(user => ({
