  ...
  try {
    ...
    if (action === 'all' || action === 'migrate_colleges') {
      try {
        ...
        results.colleges = { success: true, message: 'Colleges migrated successfully' };
      } catch (error) {
        results.colleges = { success: false, error: error.message };
      }
    }
      ...
      try {
        ...
        for (const grade of grades) {
          for (const subject of subjects) {
            const questions = await questionGenerator.generateQuestionsForSubject(grade, subject, 20);
    ...
    if (action === 'all' || action === 'populate_curriculum') {
      try {
        ...
        const curriculumTopics = [
          ...
          { grade: 11, subject: 'mathematics', topic: 'Sets', subtopics: ['Set operations', 'Venn diagrams'], learning_objectives: ['Work with sets'], competency_codes: ['MATH_11_SETS'] },
          { grade: 11, subject: 'mathematics', topic: 'Relations and Functions', subtopics: ['Domain', 'Range', 'Types of functions'], learning_objectives: ['Understand functions'], competency_codes: ['MATH_11_FUNCTIONS'] },
          
          // Grade 12 Mathematics
          { grade: 12, subject: 'mathematics', topic: 'Relations and Functions', subtopics: ['Inverse functions', 'Composition'], learning_objectives: ['Advanced function concepts'], competency_codes: ['MATH_12_FUNCTIONS'] },
          { grade: 12, subject: 'mathematics', topic: 'Inverse Trigonometric Functions', subtopics: ['Domain restrictions', 'Principal values'], learning_objectives: ['Work with inverse trig functions'], competency_codes: ['MATH_12_INVERSE_TRIG'] }
        ];
        ...
        if (insertError) {
          throw new Error(`Failed to insert curriculum topics: ${insertError.message}`);
        }
        ...
        results.curriculum = { success: false, error: error.message };
      }
    }
