...
async function handleStudentApplication(
  ...
  try {
    ...
    // Get college by slug
    const { data: college, error: collegeError } = await collegeProfileService.getProfileBySlug(params.slug)
    ...
    if (checkError && checkError.code !== 'PGRST116') {
      ...
      return APIErrorHandler.createErrorResponse(parsedError.message, parsedError.statusCode)
    }
    ...
    if (sanitizedData.documents) {
      for (const [docType, fileUrl] of Object.entries(sanitizedData.documents)) {
    ...
    const applicationData: StudentApplicationInsert = {
      ...
      class_stream: sanitizedData.class_stream,
    ...
    try {
      const notificationResult = await handleNewApplicationNotification({
        ...
        studentName: sanitizedData.full_name,
      ...
      if (!notificationResult.success && notificationResult.errors.length > 0) {
        console.warn('Some college notifications failed:', notificationResult.errors)
      }
...
export const POST = withAuth(
  withRateLimit(
    handleStudentApplication,
    {
      ...
      keyGenerator: (request, context) => `apply:${context.user?.id || request.ip}`
    }
  ),
  {
    roles: 'student',
