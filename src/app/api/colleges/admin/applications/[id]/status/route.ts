...
export async function PUT(
  ...
  try {
    ...
    const { data: existingApplication, error: fetchError } = await supabase
      ...
      .select('college_id, student_id, status as current_status')
      .eq('id', applicationId)
      .single()
    ...
    if (existingApplication.college_id !== college.id) {
      return NextResponse.json({ error: 'Forbidden - Application does not belong to your college' }, { status: 403 })
    }
    ...
    try {
      ...
      const notificationResult = await handleApplicationStatusChange({
        ...
        oldStatus: existingApplication.current_status,
        newStatus: status,
        feedback,
