...
import { normalizePaginationParams, createPaginationResult } from '@/lib/utils/pagination'
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!profile || profile.role !== 'college') {
      return NextResponse.json({ error: 'Forbidden - College role required' }, { status: 403 })
    }
    ...
    const college = {
      ...
      slug: collegeProfile.colleges.slug
    }
    ...
    const result = {
      ...
      pagination: {
        ...
        hasNext: page * limit < (data.total_count || 0),
        hasPrev: page > 1
      }
    }
...
async function getFallbackApplications(request: NextRequest) {
  ...
  const result = createPaginationResult(applications || [], page, limit, count || 0)
