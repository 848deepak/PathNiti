...
export async function POST(request: NextRequest) {
  try {
    ...
    const sarthiProfile: SarthiUserProfile = {
      ...
      aptitude_results: user_profile.quiz_scores || user_profile.aptitude_results,
      location: {
      ...
      family_background: {
        income_range: user_profile.family_income ? 
          (user_profile.family_income < 300000 ? 'low' : 
           user_profile.family_income < 800000 ? 'middle' : 'high') : 'middle',
        parent_education: user_profile.parent_education,
        parent_occupation: user_profile.parent_occupation,
        family_expectations: user_profile.family_expectations || ['job_security', 'good_salary']
      },
    ...
    switch (request_type) {
      case 'full_recommendation':
        recommendations = await sarthiAI.getEnhancedRecommendations(sarthiProfile);
        ...
        if (recommendations.primary_recommendation) {
          ...
          recommendations.primary_recommendation.nearby_colleges = nearbyColleges.slice(0, 5);
        }
        break;

      case 'stream_comparison':
        // Compare specific streams requested by user/parent
        const streamsToCompare = body.streams || ['science', 'commerce', 'arts'];
        const streamRecommendations = await sarthiAI.getEnhancedRecommendations(sarthiProfile);
        
        recommendations = {
          comparison: streamsToCompare.map((stream: string) => {
            const streamRec = streamRecommendations.alternatives.find(r => r.stream === stream) ||
                             (streamRecommendations.primary_recommendation.stream === stream ? 
                              streamRecommendations.primary_recommendation : null);
            return streamRec;
          }).filter(Boolean),
      ...
      case 'quick_advice':
        ...
        recommendations = await sarthiAI.getEnhancedRecommendations(sarthiProfile);
        recommendations.quick_response = `Based on your profile, ${recommendations.parent_friendly_summary}`;
        break;
    ...
    const response = {
      ...
      metadata: {
        ...
        recommendation_confidence: recommendations.primary_recommendation?.confidence || 0,
        ai_enhanced: true,
        sarthi_version: '1.0',
    ...
    console.error('Error in Kiro recommendations API:', error);
    return NextResponse.json(
      { 
        success: false,
...
export async function GET(_request: NextRequest) {
  return NextResponse.json({
    ...
    endpoints: {
      POST: {
        '/api/sarthi/recommendations': 'Get comprehensive career recommendations',
        body: {
          user_profile: 'User profile with interests, aptitude, family background',
          request_type: 'full_recommendation | stream_comparison | quick_advice'
        }
      }
    },
    features: [
