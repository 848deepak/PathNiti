...
export async function POST(request: NextRequest) {
  try {
    ...
    const questions = await generator.generateQuestionsForSubject(grade, subject, count);

    // Validate questions before storing
    const validationResults = questions.map(q => generator.validateQuestion(q));
    ...
    const validQuestions = questions.filter((_, index) => validationResults[index].isValid);
