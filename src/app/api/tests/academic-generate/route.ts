...
export async function POST(request: NextRequest) {
  try {
    ...
    const testQuestions = await generateTestQuestions(grade, subjects, total_questions, supabase);
    ...
    const totalMarks = testQuestions.reduce((sum, q) => sum + (q.scoring_weight || 1), 0);
    const totalTime = testQuestions.reduce((sum, q) => sum + (q.time_limit || 60), 0);
...
async function generateTestQuestions(grade: number, subjects: string[], totalQuestions: number, supabase: any) {
  ...
  for (let i = 0; i < subjects.length; i++) {
    ...
    const questionCount = questionsPerSubject + (i < remainingQuestions ? 1 : 0);
    ...
    if (subjectQuestions && subjectQuestions.length > 0) {
      ...
      await generateQuestionsForSubject(grade, subject, questionCount, supabase);
...
async function generateQuestionsForSubject(grade: number, subject: string, count: number, supabase: any) {
