...
import { createServerDocumentStorageService, type StorageBucket, type DocumentType } from '@/lib/services/document-storage-service'
import { validateFileComprehensive, sanitizeFileName } from '@/lib/utils/file-validation'
import { withAuth, withRateLimit, AuthContext } from '@/lib/auth/security-middleware'
...
async function handleFileUpload(request: NextRequest, authContext: AuthContext) {
  ...
  try {
    ...
    await auditLogger.logFileOperation('upload', file?.name || 'unknown', auditContext, {
      bucket,
      folder,
      documentType,
      fileSize: file?.size,
    ...
    const validBuckets: StorageBucket[] = ['student-documents', 'college-documents', 'profile-images']
    ...
    let securityConfig = FILE_SECURITY_CONFIGS.documents
    if (bucket === 'profile-images') {
      securityConfig = FILE_SECURITY_CONFIGS.avatars
    } else if (documentType && ['photo', 'image'].includes(documentType.toLowerCase())) {
      securityConfig = FILE_SECURITY_CONFIGS.images
    }
    ...
    const secureFile = new File([file], securityResult.fileName!, { type: file.type })
    ...
    const permissionCheck = await serverDocumentStorageService.validateUploadPermissions(
    ...
    if (bucket === 'college-documents' && !authContext.hasRole(['college', 'admin'])) {
      await auditLogger.logSecurityEvent('unauthorized_access', auditContext, {
    ...
    if (bucket === 'student-documents' && !authContext.hasRole(['student', 'admin'])) {
      await auditLogger.logSecurityEvent('unauthorized_access', auditContext, {
    ...
    await auditLogger.logFileOperation('upload', secureFile.name, auditContext, {
      bucket,
      folder,
      documentType,
      fileSize: file.size,
    ...
    const response: any = {
      ...
      data: {
        ...
        securityInfo: {
          fileHash: securityResult.fileHash,
          virusScanClean: securityResult.virusScanResult?.isClean,
          scanEngine: securityResult.virusScanResult?.scanEngine
        }
      },
...
async function handleFileDelete(request: NextRequest, authContext: AuthContext) {
  const auditContext = extractAuditContext(request, authContext.user?.id)
  const serverDocumentStorageService = createServerDocumentStorageService()
...
async function handleFileList(request: NextRequest, authContext: AuthContext) {
  ...
  try {
    ...
    if (userId && userId !== authContext.user?.id && !authContext.hasRole(['admin'])) {
      await auditLogger.logSecurityEvent('unauthorized_access', auditContext, {
    ...
    const result = await serverDocumentStorageService.getUserDocuments(userId || undefined, filters)
...
export const POST = withAuth(
  withRateLimit(
    handleFileUpload,
    {
      ...
      keyGenerator: (request, context) => `upload:${context.user?.id || request.ip}`
    }
  ),
  { requireAuth: true }
)
...
export const DELETE = withAuth(
  withRateLimit(
    handleFileDelete,
    {
      ...
      keyGenerator: (request, context) => `delete:${context.user?.id || request.ip}`
    }
  ),
  { requireAuth: true }
)
...
export const GET = withAuth(
  withRateLimit(
    handleFileList,
    {
      ...
      keyGenerator: (request, context) => `list:${context.user?.id || request.ip}`
    }
  ),
  { requireAuth: true }
)