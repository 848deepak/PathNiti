...
import { CheckCircle, Clock, Brain, Heart, Users, BookOpen, MapPin, DollarSign } from 'lucide-react';
...
const ASSESSMENT_SECTIONS = {
  aptitude: {
    ...
    description: 'Test your logical reasoning, quantitative skills, and cognitive abilities',
    icon: Brain,
    color: 'text-blue-600'
  },
  riasec: {
  ...
  constraints: {
    ...
    description: 'Tell us about your location, finances, and family expectations',
    icon: MapPin,
...
export default function ComprehensiveAssessmentPage() {
  ...
  const completedSections = Object.keys(sectionProgress).filter(section => sectionProgress[section] === 100).length;
  ...
  const calculateScores = useCallback(() => {
    ...
    Object.entries(answers).forEach(([questionId, answer]) => {
      const question = Object.values(SAMPLE_QUESTIONS).flat().find(q => q.id === questionId);
      if (!question) return;
      ...
      switch (question.type) {
        case 'aptitude':
          if (question.category in aptitudeScores) {
            aptitudeScores[question.category as keyof typeof aptitudeScores] = normalizedScore;
          }
          break;
        case 'riasec_interest':
          if (question.category in riasecScores) {
            riasecScores[question.category as keyof typeof riasecScores] = normalizedScore;
          }
          break;
        case 'personality':
          if (question.category in personalityScores) {
            personalityScores[question.category as keyof typeof personalityScores] = normalizedScore;
          }
          break;
        case 'subject_performance':
          if (question.category in subjectPerformance) {
            const category = question.category as keyof typeof subjectPerformance;
            if (question.subcategory === 'accuracy') {
              ...
              subjectPerformance[category].speed = Math.max(0, 1 - (timeTaken / question.timeLimit));
            }
          }
          break;
      }
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-3xl font-bold text-gray-900">Comprehensive Career Assessment</h1>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Help us understand your practical considerations for career planning
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div>
              <Label htmlFor="location">Preferred Location for Studies/Career</Label>
              <Select value={practicalConstraints.location} onValueChange={(value) => 
                setPracticalConstraints(prev => ({ ...prev, location: value }))
              }>
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="anywhere_india">Anywhere in India</SelectItem>
                  <SelectItem value="metro_cities">Metro cities only</SelectItem>
            ...
            <div>
              ...
              <Select value={practicalConstraints.financial_background} onValueChange={(value) => 
                setPracticalConstraints(prev => ({ ...prev, financial_background: value }))
              }>
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="middle">Middle income (3-8 LPA)</SelectItem>
                  <SelectItem value="upper_middle">Upper middle (8-20 LPA)</SelectItem>
                  <SelectItem value="high">High income (&gt; 20 LPA)</SelectItem>
                </SelectContent>
              </Select>
            </div>

            <div>
              <Label htmlFor="parental">Primary Parental/Family Expectation</Label>
              <Select value={practicalConstraints.parental_expectation} onValueChange={(value) => 
                setPracticalConstraints(prev => ({ ...prev, parental_expectation: value }))
              }>
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="government_job">Secure government job</SelectItem>
                  <SelectItem value="business">Start own business</SelectItem>
                  <SelectItem value="stable_career">Any stable, well-paying career</SelectItem>
                  <SelectItem value="follow_passion">Follow your passion</SelectItem>
                  <SelectItem value="no_pressure">No specific expectation</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="pt-6">
              <Button 
                ...
                className="w-full" 
                size="lg"
                disabled={isSubmitting || !practicalConstraints.location || !practicalConstraints.financial_background || !practicalConstraints.parental_expectation}
              >
                {isSubmitting ? 'Generating Recommendations...' : 'Submit Assessment & Get Recommendations'}
              </Button>
            </div>
          </CardContent>
        </Card>
  ...
  const SectionIcon = ASSESSMENT_SECTIONS[currentSection]?.icon || Brain;
  const sectionColor = ASSESSMENT_SECTIONS[currentSection]?.color || 'text-gray-600';
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mt-8 flex justify-between">
            ...
            >
              {currentQuestionIndex === currentQuestions.length - 1 ? 'Complete Section' : 'Next Question'}
            </Button>
          </div>
        </CardContent>
      </Card>
