  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/college/analytics?college_id=${profile.college_id}`)
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold text-gray-900">College Analytics Dashboard</h1>
            </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold mb-2">
                    Welcome, {profile?.college_name || 'College'}!
                  </h2>
                  <p className="text-green-100">
                    Monitor your college's visibility, student engagement, and application metrics.
                  </p>
                </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Student Interests</CardTitle>
            ...
            <CardContent>
              {analyticsLoading ? (
                ...
                <>
                  <div className="text-2xl font-bold">{analytics?.totalInterests || 0}</div>
          ...
          <Card>
            ...
            <CardContent>
              {analyticsLoading ? (
                ...
                <>
                  <div className="text-2xl font-bold">{analytics?.totalPrograms || 0}</div>
                  <p className="text-xs text-muted-foreground">
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Student interaction and interest trends</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium">Interest Growth Rate</span>
                      <span className="text-sm text-green-600">
                        {analyticsLoading ? '...' : `+${Math.round(analytics?.engagementMetrics?.interestGrowthRate || 0)}%`}
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className="bg-green-600 h-2 rounded-full" 
                        style={{ width: `${Math.min(analytics?.engagementMetrics?.interestGrowthRate || 0, 100)}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm text-blue-600">
                        {analyticsLoading ? '...' : `${Math.round(analytics?.engagementMetrics?.applicationConversionRate || 0)}%`}
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className="bg-blue-600 h-2 rounded-full" 
                        style={{ width: `${Math.min(analytics?.engagementMetrics?.applicationConversionRate || 0, 100)}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium">Average Program Fees</span>
                      <span className="text-sm text-purple-600">
                        {analyticsLoading ? '...' : `₹${(analytics?.engagementMetrics?.averageProgramFees || 0).toLocaleString()}`}
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
            ...
            <CardContent>
              {analyticsLoading ? (
                ...
                <div className="flex items-center justify-center py-8">
                  ...
                  <span className="text-sm text-red-500">Failed to load programs</span>
                </div>
              ) : (
                <div className="space-y-4">
                  {analytics?.streamDistribution && Object.keys(analytics.streamDistribution).length > 0 ? (
                    Object.entries(analytics.streamDistribution).map(([stream, count], index) => {
                      ...
                      return (
                        <div key={stream} className="flex items-center justify-between">
                          <span className="text-sm font-medium">{stream.replace('_', ' ').toUpperCase()}</span>
                          <span className={`text-sm ${colors[index % colors.length]}`}>{count} programs</span>
                        </div>
                      );
                    })
                  ) : (
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">Neighboring States</span>
              ...
              <div className="space-y-4">
                {analytics?.recentActivity?.length ? (
                  analytics.recentActivity.map((activity, index) => (
                    <div key={index} className="flex items-center space-x-3">
                      <div className={`w-2 h-2 rounded-full ${
                        ...
                        activity.action.includes('program') ? 'bg-blue-600' :
                  ...
                  <div className="text-center py-4">
                    ...
                    <p className="text-xs text-gray-400 mt-1">Student interactions will appear here</p>
                  </div>
                )}
              </div>
            )}
