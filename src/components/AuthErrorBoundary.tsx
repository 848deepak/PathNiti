...
export class AuthErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    if (this.props.autoRetry && parsedError.retryable && this.state.retryCount < (this.props.maxRetries || 3)) {
...
export function useAuthErrorHandler() {
  const handleAuthError = (error: Error, context?: any) => {
    const parsedError = parseAuthError(error)
  ...
  const handleAsyncAuthError = async (asyncFn: () => Promise<any>, context?: any) => {
    try {
 ...
 */
export function ProtectedRouteErrorBoundary({ children }: { children: ReactNode }) {
  return (
    <AuthErrorBoundary
      ...
      onError={(error) => {
        ...
        if (parsedError.type === 'session_expired' || parsedError.type === 'session_invalid') {
          setTimeout(() => {
            window.location.href = '/auth/login'
          }, 1000)
        }
