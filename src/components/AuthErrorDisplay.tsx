...
import { AlertTriangle, RefreshCw, LogIn, UserPlus, Settings, HelpCircle } from 'lucide-react'
...
export function AuthErrorDisplay({
  ...
  const getActionButton = () => {
    ...
    const actionConfig = {
      retry: { label: 'Try Again', icon: RefreshCw, variant: 'default' as const },
      login: { label: 'Sign In', icon: LogIn, variant: 'default' as const },
      signup: { label: 'Sign Up', icon: UserPlus, variant: 'default' as const },
      complete_profile: { label: 'Complete Profile', icon: Settings, variant: 'default' as const },
      contact_support: { label: 'Contact Support', icon: HelpCircle, variant: 'outline' as const }
    }
  ...
  if (variant === 'inline') {
    return (
      <div className={`flex items-center space-x-3 p-3 rounded-lg border ${
        getErrorSeverity() === 'error' ? 'border-red-200 bg-red-50' :
        getErrorSeverity() === 'warning' ? 'border-orange-200 bg-orange-50' :
  ...
  if (variant === 'banner') {
    return (
      <div className={`border-l-4 p-4 ${
        getErrorSeverity() === 'error' ? 'border-red-400 bg-red-50' :
        getErrorSeverity() === 'warning' ? 'border-orange-400 bg-orange-50' :
    ...
    <Card className={`${className}`}>
      <CardHeader className="pb-3">
        <div className="flex items-center space-x-2">
          ...
          <Badge variant={getErrorSeverity() === 'error' ? 'destructive' : 'secondary'}>
...
export function ProfileIncompleteError({ onComplete }: { onComplete?: () => void }) {
  const error: AuthErrorInfo = {
