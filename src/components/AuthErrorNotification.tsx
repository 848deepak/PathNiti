            ...
            {errorHistory.slice(0, maxVisible).map((error, index) => (
              ...
              >
                <ErrorNotificationCard
                  ...
                  onClose={() => {
                    // Remove from history
                    setVisibleErrors(prev => prev.filter(id => id !== `${error.type}-${index}`))
                  }}
                  isClosable={true}
                  isHistorical={true}
                />
...
function ErrorNotificationCard({
  ...
  return (
    <div className={`rounded-lg border p-4 shadow-lg ${getSeverityColors()} ${isHistorical ? 'opacity-75' : ''}`}>
      <div className="flex items-start">
        ...
        <div className="ml-3 flex-1">
          ...
          {!isHistorical && (error.retryable || error.action) && (
              ...
              {error.action && error.action !== 'retry' && (
                ...
                >
                  {error.action === 'login' ? 'Sign In' :
                   error.action === 'signup' ? 'Sign Up' :
                   error.action === 'complete_profile' ? 'Complete Profile' :
                   error.action === 'contact_support' ? 'Contact Support' :
                   'Take Action'}
                </button>
              )}
            </div>
        ...
        <div className="flex items-center justify-between flex-wrap">
          ...
          <div className="order-3 mt-2 flex-shrink-0 w-full sm:order-2 sm:mt-0 sm:w-auto">
            {currentError.action && (
              ...
              >
                {currentError.action === 'login' ? 'Sign In' : 'Complete Profile'}
              </button>
            )}
          </div>
