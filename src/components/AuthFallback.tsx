...
import { AlertTriangle, LogIn, Settings, RefreshCw, Wifi, WifiOff } from 'lucide-react'
...
interface AuthFallbackProps {
  type?: 'loading' | 'unauthenticated' | 'profile_incomplete' | 'network_error' | 'permission_denied' | 'session_expired'
  title?: string
  message?: string
  showRetry?: boolean
  showLogin?: boolean
  ...
  if (type === 'loading' || loading) {
    return (
      <div className={`flex items-center justify-center min-h-[400px] ${className}`}>
        <AuthLoading variant="spinner" size="lg" message="Loading authentication..." />
      </div>
    )
  }
      ...
      <div className={`flex items-center justify-center min-h-[400px] ${className}`}>
        <Card className="max-w-md w-full">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              {message || 'Please complete your profile setup to access all features.'}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex flex-col space-y-2">
