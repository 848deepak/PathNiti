...
export default function CollegeCourseManager({ collegeId, collegeName }: CollegeCourseManagerProps) {
  ...
  const handleFeesChange = (field: keyof CourseFormData['fees'], value: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const requestData = {
        ...
        fees: {
          tuition: formData.fees.tuition === '' ? null : Number(formData.fees.tuition),
          other: formData.fees.other === '' ? null : Number(formData.fees.other),
          total: formData.fees.total === '' ? null : Number(formData.fees.total)
        },
      ...
      if (editingCourse) {
        ...
        response = await fetch(`/api/colleges/admin/courses/${editingCourse.id}`, {
          method: 'PUT',
          headers: {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      ...
                      {course.fees?.total && (
                        <div className="flex items-center gap-2">
                          ...
                          <span>Fee: ₹{course.fees.total.toLocaleString()}</span>
                        </div>
                      )}
                    </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <Label htmlFor="tuition" className="text-sm">Tuition Fee</Label>
                  <Input
                    id="tuition"
                    type="number"
                    min="0"
                ...
                <div>
                  <Label htmlFor="other" className="text-sm">Other Fees</Label>
                  <Input
                    id="other"
                    type="number"
                    min="0"
                ...
                <div>
                  <Label htmlFor="total" className="text-sm">Total Fee</Label>
                  <Input
                    id="total"
                    type="number"
                    min="0"
