  ...
  return (
    <div className={`space-y-2 ${className}`}>
      ...
      <div className="space-y-1">
        <div className={`text-xs flex items-center ${checks.length ? 'text-green-600' : 'text-gray-500'}`}>
          <span className="mr-2">{checks.length ? '✓' : '○'}</span>
          At least 8 characters
        </div>
        <div className={`text-xs flex items-center ${checks.lowercase ? 'text-green-600' : 'text-gray-500'}`}>
          <span className="mr-2">{checks.lowercase ? '✓' : '○'}</span>
          One lowercase letter
        </div>
        <div className={`text-xs flex items-center ${checks.uppercase ? 'text-green-600' : 'text-gray-500'}`}>
          <span className="mr-2">{checks.uppercase ? '✓' : '○'}</span>
          One uppercase letter
        </div>
        <div className={`text-xs flex items-center ${checks.number ? 'text-green-600' : 'text-gray-500'}`}>
          <span className="mr-2">{checks.number ? '✓' : '○'}</span>
          One number
        </div>
        <div className={`text-xs flex items-center ${checks.special ? 'text-green-600' : 'text-gray-500'}`}>
