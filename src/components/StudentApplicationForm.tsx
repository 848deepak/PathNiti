...
import { Button, Card, CardContent, CardHeader, CardTitle, Input, Label, Textarea, Progress, Alert, AlertDescription } from "@/components/ui"
import { createClient } from "@/lib/supabase/client"
import { validateFile, generateUniqueFilename, formatFileSize, ALLOWED_FILE_TYPES, MAX_FILE_SIZE } from "@/lib/utils/file-validation"
...
export default function StudentApplicationForm({ 
  ...
  const [documents, setDocuments] = useState({
    marksheet_10th: { file: null, url: null, uploading: false, progress: 0, error: null } as DocumentUpload,
    marksheet_12th: { file: null, url: null, uploading: false, progress: 0, error: null } as DocumentUpload,
  ...
  const handleFileSelect = async (
    ...
    if (!validationResult.isValid) {
      setErrors(prev => ({ ...prev, [documentType]: validationResult.error || 'Invalid file' }))
      return
    }
    ...
    if (documentType === 'other_documents') {
      ...
      const { url, error } = await uploadFile(file, `other-documents/${collegeId}`)
  ...
  const removeDocument = (documentType: 'marksheet_10th' | 'marksheet_12th', index?: number) => {
    if (documentType === 'other_documents' && typeof index === 'number') {
      ...
      setDocuments(prev => ({
        ...prev,
        [documentType]: { file: null, url: null, uploading: false, progress: 0, error: null }
      }))
    }
  }
  ...
  const renderFileUpload = (
    ...
    return (
      <div className="space-y-2">
        ...
        {!isMultiple && (document as DocumentUpload).file && (
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-700">{(document as DocumentUpload).file?.name}</span>
        ...
        {isMultiple && (documents.other_documents as DocumentUpload[]).map((doc, index) => (
        ...
        {!isMultiple && (document as DocumentUpload).uploading && (
          <Progress value={(document as DocumentUpload).progress} className="w-full" />
        )}
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="class_stream">Class/Stream <span className="text-red-500">*</span></Label>
              <Input
                id="class_stream"
                type="text"
                value={formData.class_stream}
                onChange={(e) => setFormData(prev => ({ ...prev, class_stream: e.target.value }))}
