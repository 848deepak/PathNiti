...
export function StudentApplicationTracker({ userId }: StudentApplicationTrackerProps) {
  const [applications, setApplications] = useState<ApplicationWithCollege[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [updatingDocuments, setUpdatingDocuments] = useState<string | null>(null)
  ...
  const handleDocumentUpdate = async (applicationId: string) => {
    ...
    fileInput.onchange = async (event) => {
      ...
      try {
        ...
        const documents = {
          marksheet_10th: uploadedFiles.find(f => f.name.toLowerCase().includes('10th') || f.name.toLowerCase().includes('tenth'))?.url || '',
          marksheet_12th: uploadedFiles.find(f => f.name.toLowerCase().includes('12th') || f.name.toLowerCase().includes('twelfth'))?.url || '',
          other_documents: uploadedFiles.filter(f => 
        ...
        const updateResponse = await fetch(`/api/student/applications/${applicationId}/documents`, {
          method: 'PUT',
          headers: {
        ...
        setError(error instanceof Error ? error.message : 'Failed to update documents')
      } finally {
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                      <h3 className="font-semibold text-lg">{application.college_name}</h3>
