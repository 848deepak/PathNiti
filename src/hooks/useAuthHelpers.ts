...
export function useAuthHelpers() {
  ...
  // Role helpers
  const checkRole = useCallback((role: 'student' | 'admin' | 'college') => {
  ...
  const requireRole = useCallback((role: 'student' | 'admin' | 'college') => {
  ...
  const requireStudent = useCallback(() => requireRole('student'), [requireRole])
  const requireCollege = useCallback(() => requireRole('college'), [requireRole])
