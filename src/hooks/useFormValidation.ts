...
import { FormValidator, FormValidationState } from '../lib/utils/form-validation'
...
export function useFormValidation(
  ...
  const [formState, setFormState] = useState<FormState>(() => {
    const fields = [
      ...
      'phone', 'collegeId', 'contactPerson', 'designation'
    ]
    
    const initialState: FormState = {}
    fields.forEach(field => {
      initialState[field] = {
        value: (initialData as any)[field] || '',
        touched: false,
        isValid: true
      }
    })
  ...
  const [validationState, setValidationState] = useState<FormValidationState | null>(null)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [submitError, setSubmitError] = useState<string | null>(null)
  const [sessionRecoveryAvailable, setSessionRecoveryAvailable] = useState(false)
  ...
  const validateForm = useCallback((immediate = false) => {
    ...
    const performValidation = () => {
      ...
      setFormState(prevState => {
        ...
        Object.keys(updatedState).forEach(field => {
          const fieldValidation = validation[field as keyof FormValidationState]
          if (fieldValidation) {
            updatedState[field] = {
              ...updatedState[field],
  ...
  const updateField = useCallback((fieldName: string, value: string) => {
    ...
    if (enableSessionRecovery && fieldName !== 'password' && fieldName !== 'confirmPassword') {
      ...
      if (Object.values(currentData).some(val => val && val.toString().trim() !== '')) {
  ...
  }, [validateOnChange, validateForm, submitError, enableSessionRecovery, formState])
  ...
  const hasWarnings = useMemo(() => {
    return Object.values(formState).some(field => field.warnings && field.warnings.length > 0)
  }, [formState])
