...
export interface AuthErrorInfo {
  ...
  retryable: boolean
  redirectTo?: string
  action?: 'retry' | 'login' | 'signup' | 'complete_profile' | 'contact_support'
}
...
export function parseAuthError(error: any): AuthErrorInfo {
  ...
  if (message.includes('password') && (message.includes('weak') || message.includes('strength'))) {
    return {
      ...
      message: error.message,
      userMessage: 'Password is too weak. Please use at least 8 characters with uppercase, lowercase, and numbers.',
      recoverable: true,
      retryable: true,
      action: 'retry'
    }
...
export function getErrorMessage(errorType: AuthErrorType): string {
  const errorMessages: Record<AuthErrorType, string> = {
    ...
    [AuthErrorType.WEAK_PASSWORD]: 'Password is too weak. Please use at least 8 characters with uppercase, lowercase, and numbers.',
    [AuthErrorType.EMAIL_ALREADY_EXISTS]: 'An account with this email already exists. Try signing in instead.',
    ...
    [AuthErrorType.TOKEN_REFRESH_FAILED]: 'Session refresh failed. Please sign in again.',
    [AuthErrorType.PROFILE_NOT_FOUND]: 'Profile not found. Please complete your profile setup.',
    [AuthErrorType.PROFILE_CREATION_FAILED]: 'Failed to create profile. Please try again.',
    [AuthErrorType.PROFILE_UPDATE_FAILED]: 'Failed to update profile. Please try again.',
    [AuthErrorType.PROFILE_INCOMPLETE]: 'Please complete your profile to continue.',
    [AuthErrorType.INSUFFICIENT_PERMISSIONS]: 'You don\'t have permission to access this resource.',
    [AuthErrorType.ROLE_MISMATCH]: 'Your account role doesn\'t match the required permissions.',
    [AuthErrorType.ACCESS_DENIED]: 'Access denied. Please contact support if you believe this is an error.',
    [AuthErrorType.NETWORK_ERROR]: 'Network error. Please check your internet connection and try again.',
    [AuthErrorType.TIMEOUT_ERROR]: 'Request timed out. Please try again.',
    [AuthErrorType.SERVER_ERROR]: 'Server error. Please try again in a few moments.',
    [AuthErrorType.UNKNOWN_ERROR]: 'An unexpected error occurred. Please try again or contact support if the problem persists.',
 ...
 */
export function getRedirectUrl(errorInfo: AuthErrorInfo, currentPath?: string): string {
 ...
 */
export function logAuthError(errorInfo: AuthErrorInfo, context?: any) {
  const logData = {
