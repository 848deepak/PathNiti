...
class AuthPerformanceMonitor {
  ...
  startTimer(operation: string) {
    ...
    return {
        ...
        if (process.env.NODE_ENV === 'development') {
          ...
          console.log(`[Auth Performance] ${status} ${operation}: ${duration.toFixed(2)}ms`)
          if (error) {
  ...
  getOverallStats() {
    ...
    const operationStats = operations.map(op => this.getOperationStats(op)).filter((op): op is NonNullable<typeof op> => op !== null)
...
export function useAuthPerformance() {
  return {
    getStats: (operation?: string) => 
      operation ? authPerformanceMonitor.getOperationStats(operation) : authPerformanceMonitor.getOverallStats(),
    getSlowOperations: (threshold?: number) => authPerformanceMonitor.getSlowOperations(threshold),
    getRecentErrors: (limit?: number) => authPerformanceMonitor.getRecentErrors(limit),
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    if (stats) {
      ...
      stats.operations.forEach(op => {
        if (op) {
          console.log(`  ${op.operation}: ${op.averageDuration.toFixed(2)}ms avg (${op.totalCalls} calls)`)
        }
      })
