...
export function withRateLimit(
  ...
  return async (request: NextRequest, context: AuthContext, params?: any) => {
    const key = options.keyGenerator 
      ? options.keyGenerator(request, context)
      : context.user?.id || request.ip || 'anonymous'
    ...
    const current = rateLimitMap.get(key) || { count: 0, resetTime: now + options.windowMs }
...
export function withValidation<T>(
  handler: (request: NextRequest, context: AuthContext, validatedData: T, params?: any) => Promise<NextResponse>,
  schema: {
  ...
  return async (request: NextRequest, context: AuthContext, params?: any) => {
    try {
      ...
      if (!validation.success) {
        return NextResponse.json(
          { error: 'Validation failed', code: 'VALIDATION_ERROR', details: validation.error },
          { status: 400 }
        )
      }
