...
export class ChatSessionManager {
   ...
   */
  async createSession(userId: string, sessionName: string = 'Chat with Sarthi'): Promise<ChatSession> {
   ...
   */
  async getConversationContext(sessionId: string, messageLimit: number = 10): Promise<{
   ...
   */
  async addUserFeedback(interactionId: string, feedback: string): Promise<void> {
    const { error } = await this.supabase
      .from('sarthi_interactions')
      .update({
   ...
   */
  async getAnalyticsData(startDate?: string, endDate?: string): Promise<{
    ...
    interactions?.forEach(interaction => {
      ...
      capabilityUsage[interaction.capability_used] = 
        (capabilityUsage[interaction.capability_used] || 0) + 1;
      ...
      totalProcessingTime += interaction.processing_time_ms || 0;
    });
    ...
    return {
      ...
      averageProcessingTime: totalInteractions > 0 ? totalProcessingTime / totalInteractions : 0,
    };
  }
}
