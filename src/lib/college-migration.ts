...
export class CollegeMigrationService {
  ...
  async migrateMockColleges(): Promise<void> {
    ...
    try {
      ...
      const { data: existingColleges, error: checkError } = await this.supabase
        ...
        .select('college_id')
        .in('college_id', mockColleges.map(c => c.college_id));

      if (checkError) {
        throw new Error(`Failed to check existing colleges: ${checkError.message}`);
      }

      const existingIds = existingColleges?.map(c => c.college_id) || [];
      const newColleges = mockColleges.filter(c => !existingIds.includes(c.college_id));
      ...
      if (newColleges.length > 0) {
        ...
        console.log(`Successfully migrated ${newColleges.length} colleges to database`);
      } else {
  ...
  // Update college data
  async updateCollege(collegeId: string, updates: Partial<CollegeData>): Promise<void> {
    try {
  ...
  async verifyCollege(collegeId: string, verified: boolean): Promise<void> {
    try {
      ...
      console.log(`Successfully ${verified ? 'verified' : 'unverified'} college ${collegeId}`);
  ...
  // Search colleges
  async searchColleges(searchTerm: string, filters?: {
    streams?: string[];
    verified?: boolean;
  }): Promise<CollegeData[]> {
    try {
