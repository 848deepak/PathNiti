...
const ENHANCED_STREAM_DATA = [
  {
    ...
    careers: [
      { name: 'Engineering', fast_earning: true, govt_jobs: true, stability: 'high' },
      { name: 'Medical', fast_earning: false, govt_jobs: true, stability: 'very_high' },
      { name: 'Research Scientist', fast_earning: false, govt_jobs: true, stability: 'medium' },
      { name: 'Data Scientist', fast_earning: true, govt_jobs: false, stability: 'high' },
      { name: 'Pharmacy', fast_earning: true, govt_jobs: true, stability: 'high' }
    ],
  ...
  },
  {
    ...
    careers: [
      { name: 'CA/CS', fast_earning: true, govt_jobs: false, stability: 'high' },
      { name: 'Banking', fast_earning: true, govt_jobs: true, stability: 'high' },
      { name: 'Business Management', fast_earning: true, govt_jobs: false, stability: 'medium' },
      { name: 'Government Services', fast_earning: true, govt_jobs: true, stability: 'very_high' },
      { name: 'Finance Analyst', fast_earning: true, govt_jobs: false, stability: 'high' }
    ],
  ...
  },
  {
    ...
    careers: [
      { name: 'Civil Services', fast_earning: true, govt_jobs: true, stability: 'very_high' },
      { name: 'Teaching', fast_earning: true, govt_jobs: true, stability: 'high' },
      { name: 'Journalism', fast_earning: false, govt_jobs: false, stability: 'medium' },
      { name: 'Law', fast_earning: false, govt_jobs: true, stability: 'high' },
      { name: 'Social Work', fast_earning: false, govt_jobs: true, stability: 'medium' }
    ],
  ...
  },
  {
    ...
    careers: [
      { name: 'Technical Trades', fast_earning: true, govt_jobs: true, stability: 'high' },
      { name: 'Digital Marketing', fast_earning: true, govt_jobs: false, stability: 'medium' },
      { name: 'Hospitality', fast_earning: true, govt_jobs: false, stability: 'medium' },
      { name: 'Healthcare Support', fast_earning: true, govt_jobs: true, stability: 'high' },
      { name: 'Entrepreneurship', fast_earning: true, govt_jobs: false, stability: 'medium' }
    ],
...
export class SarthiAI {
  ...
  constructor() {
    ...
    if (!apiKey) {
      console.warn('GEMINI_API_KEY not found. Sarthi AI will use fallback logic.');
    } else {
  ...
  async getEnhancedRecommendations(userProfile: SarthiUserProfile): Promise<{
    ...
    try {
      ...
      const basicRecommendations = this.calculateAdvancedStreamRecommendations(userProfile);
      
      // Enhance with AI if available
      if (this.model && usageMonitor.canMakeRequest()) {
        return await this.enhanceWithSarthiAI(userProfile, basicRecommendations);
      }
   ...
   */
  private calculateAdvancedStreamRecommendations(userProfile: SarthiUserProfile): SarthiStreamRecommendation[] {
    ...
    for (const streamData of ENHANCED_STREAM_DATA) {
      ...
      if (userProfile.aptitude_results) {
        for (const [subject, aptitudeScore] of Object.entries(userProfile.aptitude_results)) {
          if (streamData.subjects.some(s => s.toLowerCase().includes(subject.toLowerCase()))) {
      ...
      for (const interest of userProfile.interests) {
        if (streamData.careers.some(career => 
          career.name.toLowerCase().includes(interest.toLowerCase())
        )) {
          score += 30;
        }
        if (streamData.subjects.some(subject => 
      ...
      if (userProfile.family_background) {
        const { income_range, family_expectations } = userProfile.family_background;
        
        // If low income, prioritize cost-effective streams with fast earning
        if (income_range === 'low') {
          const fastEarningCareers = streamData.careers.filter(c => c.fast_earning);
          score += fastEarningCareers.length * 15;
          if (fastEarningCareers.length > 0) {
            concerns_addressed.push('Fast earning potential for financial support');
          }
        }
        
        // Government job preferences (common in J&K)
        const govtJobCareers = streamData.careers.filter(c => c.govt_jobs);
        if (family_expectations?.includes('government_job') || income_range === 'low') {
        ...
        const stableCareers = streamData.careers.filter(c => c.stability === 'high' || c.stability === 'very_high');
      ...
      const recommendation: SarthiStreamRecommendation = {
        ...
        reasoning: this.generateReasoning(userProfile, streamData, score),
   ...
   */
  private calculateROIScore(streamData: any): number {
    const educationCost = (streamData.avg_education_cost.min + streamData.avg_education_cost.max) / 2;
    const earlyCareerSalary = (streamData.earning_timeline.early_career.min + streamData.earning_timeline.early_career.max) / 2;
    const studyDuration = streamData.study_duration;
   ...
   */
  private calculateROIAnalysis(streamData: any): SarthiStreamRecommendation['roi_analysis'] {
    const educationCost = streamData.avg_education_cost;
    const earlyCareer = streamData.earning_timeline.early_career;
    const midCareer = streamData.earning_timeline.mid_career;
    const studyDuration = streamData.study_duration;
    
    const avgEducationCost = (educationCost.min + educationCost.max) / 2;
   ...
   */
  private generateReasoning(userProfile: SarthiUserProfile, streamData: any, score: number): string {
    const aptitudeMatch = userProfile.aptitude_results ? 
      Object.keys(userProfile.aptitude_results).filter(subject =>
        streamData.subjects.some((s: string) => s.toLowerCase().includes(subject.toLowerCase()))
      ) : [];
    
    const interestMatch = userProfile.interests.filter(interest =>
      streamData.careers.some((career: any) => career.name.toLowerCase().includes(interest.toLowerCase()))
    );
    ...
    reasoning += `. This stream offers excellent ROI with ${streamData.roi_factors.job_availability} job availability and ${streamData.roi_factors.salary_growth} salary growth potential.`;
  ...
  private getAlternatives(currentStream: string): Array<{
    ...
    const alternatives = ENHANCED_STREAM_DATA
   ...
   */
  private getStreamPros(streamData: any): string[] {
    const pros: string[] = [];
    
    if (streamData.roi_factors.job_availability === 'high') {
      pros.push('High job availability');
    }
    
    if (streamData.roi_factors.salary_growth === 'high') {
      pros.push('Excellent salary growth');
    }
    
    if (streamData.roi_factors.entrepreneurship === 'high' || streamData.roi_factors.entrepreneurship === 'very_high') {
      pros.push('Strong entrepreneurship opportunities');
    }
    
    const govtJobs = streamData.careers.filter((c: any) => c.govt_jobs).length;
    if (govtJobs > 0) {
      pros.push(`${govtJobs} government job options`);
    }
    
    if (streamData.study_duration <= 3) {
   ...
   */
  private getStreamCons(streamData: any): string[] {
    const cons: string[] = [];
    
    if (streamData.avg_education_cost.max > 1000000) {
      cons.push('Higher education costs');
    }
    
    if (streamData.study_duration > 4) {
      cons.push('Longer study duration');
    }
    
    if (streamData.roi_factors.job_availability === 'medium' || streamData.roi_factors.job_availability === 'low') {
      cons.push('Limited job opportunities');
    }
    
    if (streamData.earning_timeline.early_career.min < 300000) {
   ...
   */
  private compareROI(streamData: any): string {
   ...
   */
  private getParentAppealFactors(streamData: any): string[] {
    ...
    const fastEarning = streamData.careers.filter((c: any) => c.fast_earning).length;
    if (fastEarning > 0) {
    ...
    if (streamData.earning_timeline.mid_career.max > 2000000) {
      factors.push('Excellent long-term earning potential');
    }
    
    factors.push(`Study duration: ${streamData.study_duration} years`);
  ...
  private getCareerTimeline(career: string): string {
    const timelines: Record<string, string> = {
      'Engineering': '4 years B.Tech → Entry level job → 2-3 years experience for mid-level',
      'Medical': '5.5 years MBBS → 1 year internship → 3 years specialization',
      'CA/CS': '3 years B.Com → 3 years CA/CS → Practice/Job',
      'Civil Services': '3 years graduation → 1 year preparation → UPSC → Training',
      'Teaching': '3 years graduation → 2 years B.Ed → Government teacher job',
      'Banking': '3 years graduation → Bank exam preparation → Job',
      'Technical Trades': '1-2 years diploma/certificate → Immediate job opportunities'
    };
    
    return timelines[career] || '3-4 years education → 1-2 years experience → Career growth';
  }

  private getEarningPotential(career: string, streamData: any): string {
    const early = streamData.earning_timeline.early_career;
    const mid = streamData.earning_timeline.mid_career;
    
    return `₹${(early.min/100000).toFixed(1)}-${(early.max/100000).toFixed(1)} LPA initially, growing to ₹${(mid.min/100000).toFixed(1)}-${(mid.max/100000).toFixed(1)} LPA`;
  }
  ...
  private async enhanceWithSarthiAI(
    ...
    try {
      ...
      console.log(`Sarthi AI - Free tier usage: ${stats.requestsToday}/${stats.dailyLimit} requests today`);
      
      return this.parseSarthiResponse(text, userProfile, basicRecommendations);
    } catch (error) {
      console.error('Sarthi AI enhancement failed (falling back to basic):', error);
      return this.formatBasicRecommendations(userProfile, basicRecommendations);
    }
  }
   ...
   */
  private createSarthiPrompt(userProfile: SarthiUserProfile, recommendations: SarthiStreamRecommendation[]): string {
    return `
...
`;
   ...
   */
  private parseSarthiResponse(aiText: string, userProfile: SarthiUserProfile, basicRecommendations: SarthiStreamRecommendation[]): any {
    ...
    return {
      primary_recommendation: {
        ...
        ai_enhanced_reasoning: sections.parent_explanation || basicRecommendations[0].reasoning
      },
      alternatives: basicRecommendations.slice(1, 3).map((rec, index) => ({
        ...rec,
        ai_enhanced_reasoning: sections.alternatives || rec.reasoning
      })),
      parent_friendly_summary: sections.parent_explanation || this.generateDefaultParentSummary(basicRecommendations[0]),
      student_motivation: sections.student_motivation || this.generateDefaultStudentMotivation(basicRecommendations[0]),
      action_plan: sections.action_plan || this.generateDefaultActionPlan(userProfile, basicRecommendations[0])
    };
  }
  ...
  private extractSections(text: string): any {
    ...
    const parentMatch = text.match(/PARENT-FRIENDLY EXPLANATION[:\s]+(.*?)(?=STUDENT MOTIVATION|$)/s);
    if (parentMatch) sections.parent_explanation = parentMatch[1].trim();
    
    const studentMatch = text.match(/STUDENT MOTIVATION[:\s]+(.*?)(?=ADDRESSING ALTERNATIVES|ACTION PLAN|$)/s);
    if (studentMatch) sections.student_motivation = studentMatch[1].trim();
    
    const alternativesMatch = text.match(/ADDRESSING ALTERNATIVES[:\s]+(.*?)(?=ACTION PLAN|$)/s);
    if (alternativesMatch) sections.alternatives = alternativesMatch[1].trim();
    
    const actionMatch = text.match(/ACTION PLAN[:\s]+(.*?)$/s);
    if (actionMatch) {
      const actionText = actionMatch[1].trim();
      sections.action_plan = actionText.split(/\d+\./).filter(step => step.trim()).map(step => step.trim());
    }
   ...
   */
  private formatBasicRecommendations(userProfile: SarthiUserProfile, recommendations: SarthiStreamRecommendation[]): any {
    return {
      primary_recommendation: recommendations[0],
      alternatives: recommendations.slice(1, 3),
      parent_friendly_summary: this.generateDefaultParentSummary(recommendations[0]),
      student_motivation: this.generateDefaultStudentMotivation(recommendations[0]),
      action_plan: this.generateDefaultActionPlan(userProfile, recommendations[0])
    };
  }
   ...
   */
  private generateDefaultParentSummary(recommendation: SarthiStreamRecommendation): string {
    const roi = recommendation.roi_analysis;
    const govtJobs = recommendation.career_paths.filter(c => c.job_security === 'high').length;
   ...
   */
  private generateDefaultStudentMotivation(recommendation: SarthiStreamRecommendation): string {
    return `Your aptitude and interests align perfectly with ${recommendation.stream}, opening doors to exciting careers like ${recommendation.career_paths.slice(0, 2).map(c => c.career).join(' and ')}. This field is experiencing ${recommendation.career_paths[0]?.growth_rate || 10}% annual growth, meaning abundant opportunities for someone with your potential. You'll be solving real-world problems while building a rewarding career that matches your passions.`;
  }

  /**
   * Generate default action plan
   */
  private generateDefaultActionPlan(userProfile: SarthiUserProfile, recommendation: SarthiStreamRecommendation): string[] {
   ...
   */
  async getNearbyCollegesForStream(userProfile: SarthiUserProfile, stream: string): Promise<any[]> {
    try {
      if (!userProfile.location?.city) {
        return [];
      }

      const response = await fetch(`/api/colleges/nearby-by-stream`, {
        method: 'POST',
