...
class OfflineMapsManager {
  ...
  public async getMapTile(x: number, y: number, z: number): Promise<MapTile | null> {
    const key = `${z}/${x}/${y}`;
  ...
  private async fetchTileFromNetwork(x: number, y: number, z: number): Promise<MapTile | null> {
    try {
  ...
  private getCacheSize(): number {
    return Array.from(this.tileCache.values())
      .reduce((total, tile) => total + tile.size, 0);
  }

  public async cacheLocation(location: Omit<CachedLocation, 'id' | 'cached_at' | 'last_accessed'>): Promise<string> {
  ...
  public async getNearbyColleges(
    ...
    return colleges.filter(college => {
      const distance = this.calculateDistance(userLocation, college.coordinates);
  ...
  public async preloadMapTiles(
    ...
    for (let i = 0; i < tiles.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(({ x, y, z }) => this.getMapTile(x, y, z))
      );
  ...
  public getCacheStats(): MapCacheStats {
    ...
    const lastUpdated = Array.from(this.tileCache.values())
      .reduce((latest, tile) => Math.max(latest, tile.timestamp), 0);
