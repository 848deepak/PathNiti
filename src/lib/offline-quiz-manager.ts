...
class OfflineQuizManager {
  ...
  private async loadQuestionsCache(): Promise<void> {
    ...
    if (this.questionsCache.length > 0 && (now - this.cacheTimestamp) < this.CACHE_DURATION) {
      return;
    }
  ...
  private async cacheQuestionsForOffline(questions: QuizQuestion[]): Promise<void> {
    try {
      // Store questions in localStorage for offline access
      localStorage.setItem('quiz_questions_cache', JSON.stringify({
        questions,
        timestamp: Date.now(),
      }));
    } catch (error) {
  ...
  public async getQuestionsForSession(
    ...
    if (sessionType === 'aptitude_only') {
      filteredQuestions = this.questionsCache.filter(q => q.question_type === 'aptitude');
    } else if (sessionType === 'interest_only') {
      filteredQuestions = this.questionsCache.filter(q => q.question_type === 'riasec_interest');
    } else if (sessionType === 'personality_only') {
      filteredQuestions = this.questionsCache.filter(q => q.question_type === 'personality');
    }
  ...
  private calculateCorrectness(response: QuizResponse): boolean | undefined {
    const question = this.questionsCache.find(q => q.id === response.question_id);
  ...
  private async updateSessionProgress(sessionId: string, timeSpent: number): Promise<void> {
    try {
  ...
  public async completeQuizSession(
    ...
    try {
      ...
      const recommendations = await this.generateRecommendations(scores, practicalConstraints);
  ...
  private calculateScores(responses: any[]): {
    ...
    const subjectScores: Record<string, { accuracy: number[]; speed: number[] }> = {};

    responses.forEach(response => {
      const question = this.questionsCache.find(q => q.id === response.question_id);
      if (!question) return;

      const score = response.user_answer + 1; // Convert 0-based to 1-based scoring

      if (question.question_type === 'aptitude') {
        if (!aptitudeScores[question.category]) aptitudeScores[question.category] = [];
        aptitudeScores[question.category].push(score);
      } else if (question.question_type === 'riasec_interest') {
        if (!riasecScores[question.category]) riasecScores[question.category] = [];
        riasecScores[question.category].push(score);
      } else if (question.question_type === 'personality') {
        if (!personalityScores[question.category]) personalityScores[question.category] = [];
    ...
    const calculateAverage = (scores: number[]) => 
      scores.length > 0 ? Math.round((scores.reduce((a, b) => a + b, 0) / scores.length) * 20) : 50;
    ...
    const total_score = Math.round(
      (Object.values(aptitude_scores).reduce((a, b) => a + b, 0) +
       ...
       Object.values(personality_scores).reduce((a, b) => a + b, 0)) / 3
    );
  ...
  private async generateRecommendations(
    ...
    const topInterest = Object.entries(riasecScores)
      .sort(([,a], [,b]) => b - a)[0];
    ...
    const topAptitude = Object.entries(aptitudeScores)
      .sort(([,a], [,b]) => b - a)[0];
