...
export class QualityValidator {
  ...
  private validateQuestionText(
    ...
    if (readingLevel > expectedLevel + 2) {
      warnings.push(`Question may be too complex for grade ${question.grade} students`);
    } else if (readingLevel < expectedLevel - 2) {
      warnings.push(`Question may be too simple for grade ${question.grade} students`);
    }
  ...
  private validateAnswerFormat(
    ...
    if (['mcq_single', 'mcq_multi'].includes(question.question_type)) {
      ...
      if (question.question_type === 'mcq_single') {
        ...
        if (!Array.isArray(question.correct_answer)) {
          ...
          warnings.push('Multi-select MCQ should have at least 2 correct answers');
        }
      }
    }
    ...
    if (question.question_type === 'numerical') {
      if (typeof question.correct_answer === 'string') {
        ...
        const allNumbers = question.correct_answer.every(ans => this.isValidNumber(ans));
        if (!allNumbers) {
  ...
  private validateDifficulty(
    ...
    if (grade === 10 && difficulty === 'hard') {
      warnings.push('Grade 10 questions should generally be easy to medium difficulty');
    }

    if (grade === 12 && difficulty === 'easy') {
      warnings.push('Grade 12 questions should generally be medium to hard difficulty');
    }
    ...
    if (complexity !== difficulty) {
      suggestions.push(`Question complexity (${complexity}) doesn't match assigned difficulty (${difficulty})`);
    }
  }
  ...
  private validateTagsAndMetadata(
    ...
    if (!question.competency_codes || question.competency_codes.length === 0) {
      suggestions.push('Consider adding competency codes for better curriculum alignment');
    }
  ...
  private hasAmbiguousLanguage(text: string): boolean {
    const ambiguousWords = [
      'some', 'many', 'few', 'several', 'often', 'sometimes',
      'usually', 'generally', 'typically', 'probably', 'maybe'
    ];
  ...
  private assessQuestionComplexity(question: QuestionTemplate): 'easy' | 'medium' | 'hard' {
    let complexity = 0;
  ...
  validateTestComposition(questions: QuestionTemplate[]): ValidationResult {
    ...
    if (easyPercent < 30 || easyPercent > 50) {
      warnings.push(`Easy questions should be 30-50% of total (currently ${easyPercent.toFixed(1)}%)`);
    }

    if (mediumPercent < 30 || mediumPercent > 50) {
      warnings.push(`Medium questions should be 30-50% of total (currently ${mediumPercent.toFixed(1)}%)`);
    }

    if (hardPercent < 10 || hardPercent > 30) {
      warnings.push(`Hard questions should be 10-30% of total (currently ${hardPercent.toFixed(1)}%)`);
    }
    ...
    if (mcqPercent > 80) {
      warnings.push('Test has too many MCQ questions - consider adding more open-ended questions');
    }

    if (mcqPercent < 20) {
      warnings.push('Test has very few MCQ questions - consider adding more for better coverage');
    }
    ...
    if (totalTime > 7200) { // 2 hours
      warnings.push('Total test time exceeds 2 hours - consider reducing time or questions');
    }

    if (totalMarks > 100) {
      warnings.push('Total marks exceed 100 - consider standardizing to 100 marks');
    }
