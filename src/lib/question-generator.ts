...
export interface QuestionTemplate {
  ...
  subject: string;
  topic: string;
  question_type: 'mcq_single' | 'mcq_multi' | 'short' | 'long' | 'numerical' | 'diagram';
...
const CURRICULUM_TOPICS = {
  10: {
    ...
    science: [
      ...
      'Magnetic Effects of Electric Current', 'Sources of Energy', 'Life Processes',
      'Control and Coordination', 'How do Organisms Reproduce', 'Heredity and Evolution',
      'Natural Resources', 'Our Environment', 'Management of Natural Resources'
    ],
    english: [
      'Reading Comprehension', 'Writing Skills', 'Grammar', 'Literature',
      'Poetry Analysis', 'Prose Analysis', 'Vocabulary', 'Communication Skills'
    ],
    social_science: [
      'Nationalism in India', 'The Making of a Global World', 'The Age of Industrialisation',
      'Print Culture and the Modern World', 'Resources and Development', 'Water Resources',
  ...
  11: {
    ...
    social_science: [
      ...
      'Confrontation of Cultures', 'The Industrial Revolution', 'Displacing Indigenous Peoples',
      'Paths to Modernisation', 'The Constitution: Why and How?', 'Rights in the Indian Constitution',
      'Election and Representation', 'Executive', 'Legislature', 'Judiciary', 'Federalism',
      'Local Governments', 'Constitution as a Living Document', 'The Philosophy of the Constitution'
    ]
  },
  12: {
    mathematics: [
      ...
      'Application of Integrals', 'Differential Equations', 'Vector Algebra', 'Three Dimensional Geometry',
    ...
    social_science: [
      ...
      'Colonialism and the Countryside', 'Rebels and the Raj', 'Colonial Cities', 'Mahatma Gandhi and the Nationalist Movement',
      'Understanding Partition', 'Framing the Constitution', 'The Cold War Era', 'The End of Bipolarity',
      'US Hegemony in World Politics', 'Alternative Centres of Power', 'Contemporary South Asia',
      'International Organisations', 'Security in Contemporary World', 'Environment and Natural Resources',
      'Globalisation', 'Challenges of Nation Building', 'Era of One Party Dominance', 'Politics of Planned Development',
      'India\'s External Relations', 'Challenges to and Restoration of the Congress System', 'The Crisis of Democratic Order',
      'Rise of Popular Movements', 'Regional Aspirations', 'Recent Developments in Indian Politics'
    ]
  }
};
...
export class QuestionGenerator {
  ...
  async generateQuestionsForSubject(
    ...
    const topics = CURRICULUM_TOPICS[grade as keyof typeof CURRICULUM_TOPICS]?.[subject as keyof typeof CURRICULUM_TOPICS[10]];
    
    if (!topics) {
      throw new Error(`No topics found for grade ${grade} and subject ${subject}`);
    }
  ...
  private async generateSingleQuestion(
    ...
    switch (subject) {
      case 'mathematics':
        questionData = this.generateMathQuestion(grade, topic, difficulty, questionType);
        break;
      case 'science':
        questionData = this.generateScienceQuestion(grade, topic, difficulty, questionType);
        break;
      case 'english':
        questionData = this.generateEnglishQuestion(grade, topic, difficulty, questionType);
        break;
      case 'social_science':
        questionData = this.generateSocialScienceQuestion(grade, topic, difficulty, questionType);
        break;
      default:
  ...
  private generateMathQuestion(
    ...
    const questions = {
      'Quadratic Equations': {
        ...
        medium: {
          ...
          options: ["D = 25, Real and equal", "D = 25, Real and distinct", "D = 49, Real and equal", "D = 49, Real and distinct"],
          correct_answer: "D = 25, Real and distinct",
          explanation: "Discriminant D = b² - 4ac = 49 - 24 = 25. Since D > 0, roots are real and distinct",
          time_seconds: 120,
          marks: 3
        },
        hard: {
          text: "If α and β are roots of x² - 6x + 8 = 0, find the equation whose roots are α² and β²",
          options: ["x² - 20x + 64 = 0", "x² - 36x + 64 = 0", "x² - 20x + 16 = 0", "x² - 36x + 16 = 0"],
          correct_answer: "x² - 20x + 64 = 0",
          explanation: "α + β = 6, αβ = 8. For new equation: sum = α² + β² = (α+β)² - 2αβ = 36 - 16 = 20, product = α²β² = 64",
          time_seconds: 180,
          marks: 4
        }
      },
    ...
    if (!topicQuestions || !topicQuestions[difficulty]) {
      ...
      return {
        ...
        competency_codes: [`${subject.toUpperCase()}_${grade}_${topic.toUpperCase().replace(/\s+/g, '_')}`]
      };
    }

    const question = topicQuestions[difficulty];
    return {
      ...question,
      tags: [topic.toLowerCase().replace(/\s+/g, '_'), difficulty],
      competency_codes: [`MATH_${grade}_${topic.toUpperCase().replace(/\s+/g, '_')}`]
    };
  }
  ...
  private generateScienceQuestion(
    ...
    const questions = {
      'Light - Reflection and Refraction': {
        ...
        medium: {
          ...
          explanation: "When light enters a denser medium (glass), its speed decreases and it bends towards the normal",
          time_seconds: 90,
          marks: 2
        }
      }
    ...
    if (!topicQuestions || !topicQuestions[difficulty]) {
      return {
        ...
        marks: 2,
        tags: [topic.toLowerCase().replace(/\s+/g, '_')],
        competency_codes: [`SCIENCE_${grade}_${topic.toUpperCase().replace(/\s+/g, '_')}`]
      };
    }
  ...
  private generateEnglishQuestion(
    ...
    return {
      ...
      marks: 2,
      tags: [topic.toLowerCase().replace(/\s+/g, '_'), difficulty],
      competency_codes: [`ENGLISH_${grade}_${topic.toUpperCase().replace(/\s+/g, '_')}`]
    };
  }
  ...
  private generateSocialScienceQuestion(
    ...
    return {
      ...
      marks: 2,
      tags: [topic.toLowerCase().replace(/\s+/g, '_'), difficulty],
      competency_codes: [`SOCIAL_${grade}_${topic.toUpperCase().replace(/\s+/g, '_')}`]
    };
  }
  ...
  async generateTest(
    ...
    if (testType === 'stream_assessment') {
      ...
      for (const subject of subjects) {
        const subjectQuestions = await this.generateQuestionsForSubject(grade, subject, 5);
      ...
      // Single subject test
      const subjectQuestions = await this.generateQuestionsForSubject(grade, testType, 20);
      await this.storeQuestions(subjectQuestions);
  ...
  validateQuestion(question: QuestionTemplate): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
