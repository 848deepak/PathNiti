...
export class AuditLogger {
   ...
   */
  async logAuth(action: 'login' | 'logout' | 'signup' | 'password_reset' | 'failed_login', context: AuditContext, metadata?: Record<string, any>): Promise<void> {
    await this.log({
   ...
   */
  async logDataAccess(action: 'read' | 'export' | 'bulk_read', tableName: string, context: AuditContext, metadata?: Record<string, any>): Promise<void> {
    await this.log({
      userId: context.userId,
      action: `data.${action}`,
      tableName,
  ...
  async logSecurityEvent(
    event: 'suspicious_activity' | 'rate_limit_exceeded' | 'unauthorized_access' | 'malware_detected',
    context: AuditContext,
    metadata?: Record<string, any>
  ): Promise<void> {
    await this.log({
   ...
   */
  async getResourceAuditLogs(tableName: string, recordId?: string, limit: number = 100): Promise<any[]> {
...
export function withAuditLog(
  ...
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      const auditContext = extractAuditContext(request, context?.user?.id)
