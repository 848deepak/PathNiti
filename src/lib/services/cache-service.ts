...
class CacheService<T> {
  ...
  getTopEntries(limit = 10): Array<{ key: string; accessCount: number; lastAccessed: number }> {
...
export const collegeProfileCache = new CacheService<any>(50, 10 * 60 * 1000) // 10 minutes TTL

// College list cache with shorter TTL for more dynamic data
export const collegeListCache = new CacheService<any>(20, 5 * 60 * 1000) // 5 minutes TTL

// Application cache with very short TTL since applications change frequently
export const applicationCache = new CacheService<any>(30, 2 * 60 * 1000) // 2 minutes TTL

// Course cache with medium TTL
export const courseCache = new CacheService<any>(40, 15 * 60 * 1000) // 15 minutes TTL

// Notice cache with short TTL
export const noticeCache = new CacheService<any>(25, 3 * 60 * 1000) // 3 minutes TTL
...
export const CacheKeys = {
  ...
  collegeList: (filters: Record<string, any>) => {
    const sortedKeys = Object.keys(filters).sort()
    const filterString = sortedKeys.map(key => `${key}:${filters[key]}`).join('|')
    return `colleges:list:${filterString}`
  },
  collegeApplications: (collegeId: string, page: number, filters: Record<string, any>) => {
    const sortedKeys = Object.keys(filters).sort()
    const filterString = sortedKeys.map(key => `${key}:${filters[key]}`).join('|')
  ...
  studentApplications: (studentId: string) => `student:${studentId}:applications`
}
...
export const CacheInvalidation = {
  ...
  invalidateApplicationData: (collegeId: string, studentId?: string) => {
    ...
    const applicationKeys = Array.from(applicationCache.getStats().size > 0 ? 
...
export const CacheWarming = {
  warmCollegeProfile: async (slug: string, fetchFunction: () => Promise<any>) => {
    if (!collegeProfileCache.has(CacheKeys.collegeProfile(slug))) {
      try {
        const data = await fetchFunction()
        collegeProfileCache.set(CacheKeys.collegeProfile(slug), data)
      } catch (error) {
        console.error(`Failed to warm cache for college profile ${slug}:`, error)
      }
    }
  },
  
  warmPopularColleges: async (slugs: string[], fetchFunction: (slug: string) => Promise<any>) => {
