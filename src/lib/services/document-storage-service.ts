...
import { validateFile, generateUniqueFilename, type FileValidationResult } from '@/lib/utils/file-validation'
import type { Database } from '@/lib/supabase/types'

export type StorageBucket = 'student-documents' | 'college-documents' | 'profile-images'
export type DocumentType = 'marksheet_10th' | 'marksheet_12th' | 'other' | 'gallery' | 'profile' | 'brochure'
...
export class DocumentStorageService {
  ...
  async uploadFile(file: File, options: UploadOptions): Promise<UploadResult> {
    try {
      ...
      if (options.replaceExisting && options.documentType && options.applicationId) {
        await this.deleteExistingDocument(options.documentType, options.applicationId)
      }
   ...
   */
  async downloadFile(filePath: string, bucket: StorageBucket): Promise<{ data: Blob | null, error: string | null }> {
    try {
   ...
   */
  async getSignedUrl(filePath: string, bucket: StorageBucket, expiresIn = 3600): Promise<{ url: string | null, error: string | null }> {
    try {
      const { data, error } = await this.supabase.storage
        .from(bucket)
  ...
  async deleteFile(documentId: string): Promise<{ success: boolean, error?: string }> {
    try {
      ...
      const { error: storageError } = await this.supabase.storage
        .from(metadata.bucket_name as StorageBucket)
   ...
   */
  private async deleteExistingDocument(documentType: DocumentType, applicationId: string): Promise<void> {
    try {
   ...
   */
  async validateUploadPermissions(bucket: StorageBucket, options: UploadOptions): Promise<{ allowed: boolean, error?: string }> {
    try {
      const { data: { user }, error: authError } = await this.supabase.auth.getUser()
      if (authError || !user) {
   ...
   */
  async getPreviewUrl(filePath: string, bucket: StorageBucket, transform?: {
    width?: number
    height?: number
    quality?: number
  }): Promise<{ url: string | null, error: string | null }> {
    try {
      let url = this.supabase.storage
        .from(bucket)
...
export const createServerDocumentStorageService = () => new DocumentStorageService(true)