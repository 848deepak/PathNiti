...
class ImageOptimizationService {
  ...
  async generateImageVariants(
    ...
    try {
      ...
      const variants = await Promise.all([
        this.optimizeImage(imageUrl, { ...opts, maxWidth: 300, maxHeight: 200 }), // thumbnail
        this.optimizeImage(imageUrl, { ...opts, maxWidth: 800, maxHeight: 600 }), // medium
        this.optimizeImage(imageUrl, { ...opts, maxWidth: 1920, maxHeight: 1080 }), // large
   ...
   */
  private async getOriginalImageInfo(imageUrl: string): Promise<OptimizedImage | null> {
    try {
...
export const ImageOptimizationUtils = {
  ...
  isOptimizationSupported: (imageUrl: string): boolean => {
    return imageUrl.includes('supabase') || imageUrl.includes('cloudinary') || imageUrl.includes('imagekit')
  }
}
