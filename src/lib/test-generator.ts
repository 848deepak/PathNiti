...
export class TestGenerator {
  ...
  async generateTest(
    ...
    const timeLimit = finalConfig.timeLimit || this.calculateOptimalTime(questions);
  ...
  // Set default configuration
  private setDefaultConfiguration(config: TestConfiguration): TestConfiguration {
    const defaults: TestConfiguration = {
      grade: config.grade,
      testType: config.testType || 'stream_assessment',
      subjects: config.subjects || ['mathematics', 'science', 'english', 'social_science'],
  ...
  private async selectQuestions(config: TestConfiguration): Promise<QuestionTemplate[]> {
    const selectedQuestions: QuestionTemplate[] = [];
    const totalQuestions = config.totalQuestions!;

    // Calculate questions per subject
    const questionsPerSubject = Math.floor(totalQuestions / config.subjects!.length);
    ...
    for (let i = 0; i < config.subjects!.length; i++) {
      ...
      const subjectQuestionCount = questionsPerSubject + (i < remainingQuestions ? 1 : 0);
  ...
  private async getQuestionsForSubject(
    ...
    if (!questions || questions.length === 0) {
      throw new Error(`No questions found for grade ${grade} and subject ${subject}`);
    }
  ...
  private selectQuestionsByDistribution(
    ...
    const mcqSingleCount = Math.floor(count * questionTypeDistribution.mcq_single / 100);
    const mcqMultiCount = Math.floor(count * questionTypeDistribution.mcq_multi / 100);
    const shortCount = Math.floor(count * questionTypeDistribution.short / 100);
    const longCount = Math.floor(count * questionTypeDistribution.long / 100);
    const numericalCount = Math.floor(count * questionTypeDistribution.numerical / 100);
    const diagramCount = count - mcqSingleCount - mcqMultiCount - shortCount - longCount - numericalCount;
    ...
    for (const target of difficultyTargets) {
      const difficultyQuestions = questions.filter(q => 
        q.difficulty === target.difficulty && !used.has(q.question_id)
      );
      
      const selectedFromDifficulty = this.shuffleArray(difficultyQuestions)
    ...
    if (selected.length < count) {
      const remainingQuestions = questions.filter(q => !used.has(q.question_id));
  ...
  private calculateSubjectCoverage(questions: QuestionTemplate[]): Record<string, number> {
  ...
  async getTest(testId: string, studentId: string): Promise<GeneratedTest | null> {
    try {
      ...
      return {
        ...
        total_marks: test.total_marks,
        time_limit_seconds: test.time_limit_seconds,
        difficulty_distribution: this.calculateDifficultyDistribution(questions || []),
  ...
  async getStudentTests(studentId: string, limit: number = 10): Promise<GeneratedTest[]> {
    try {
      ...
      for (const test of tests || []) {
        const fullTest = await this.getTest(test.test_id, studentId);
        if (fullTest) {
