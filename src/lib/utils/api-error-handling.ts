...
export class APIErrorHandler {
    ...
    return async (request: NextRequest, context?: any): Promise<NextResponse> => {
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`${request.method} ${endpoint} - ${response.status} - ${Date.now() - startTime}ms`)
        }
        ...
        if (error instanceof Error) {
          ...
          if (error.message.includes('Forbidden') || error.message.includes('403')) {
            return this.createAuthorizationErrorResponse(error.message, errorContext)
          }
          ...
          if (error.message.includes('Conflict') || error.message.includes('409')) {
            return this.createConflictErrorResponse(error.message, errorContext)
          }
        }
   ...
   */
  static parseSupabaseError(error: any): { message: string; code: string; statusCode: number } {
    if (!error) {
      return {
