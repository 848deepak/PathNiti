...
export class ErrorRecoveryManager {
  ...
  static handleSubmissionError(
    ...
    if (errorMessage.toLowerCase().includes('network') || 
      ...
      actions.push({
        ...
        action: () => {
          ...
          const body = encodeURIComponent(`I encountered an error during college signup: ${errorMessage}`)
    ...
    if (sessionData && Object.values(sessionData).some(value => value && value.toString().trim() !== '')) {
      actions.push({
  ...
  static handleSessionRecovery(): {
    ...
    const sessionAge = Math.floor((Date.now() - session.timestamp) / (60 * 1000)) // minutes
    ...
    if (session.formData && Object.values(session.formData).some(value => 
  ...
  private static formatErrorMessage(error: string): string {
    ...
    const errorMappings: Record<string, string> = {
      ...
      'College not found': 'We couldn\'t find your college in our database. You can register it as a new college.',
      'Validation failed': 'Please check all fields and fix any errors before submitting.',
    }
