...
export function validateFile(file: File, options?: FileValidationOptions): FileValidationResult {
  ...
  const maxSize = options?.maxSize || (isImageFile(file) ? FILE_SIZE_LIMITS.IMAGE : FILE_SIZE_LIMITS.DOCUMENT)
  ...
  if (expectedExtension && actualExtension !== expectedExtension) {
    warnings.push(`File extension "${actualExtension}" doesn't match file type. Expected "${expectedExtension}"`)
  }
 ...
 */
function validateImageFile(file: File, options?: FileValidationOptions): FileValidationResult {
  const warnings: string[] = []
  ...
  if (options?.requireSpecificDimensions) {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        const { width: reqWidth, height: reqHeight, tolerance = 0 } = options.requireSpecificDimensions!
...
export function validateFiles(files: File[], options?: FileValidationOptions): FileValidationResult {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (result.warnings) {
      allWarnings.push(...result.warnings.map(w => `File ${i + 1} (${file.name}): ${w}`))
    }
  }
 ...
 */
export async function validateFileIntegrity(file: File): Promise<FileValidationResult> {
  try {
...
export function validateFileUpload(file: File, options: FileUploadValidationOptions): FileUploadValidationResult {
  const errors: string[] = []
