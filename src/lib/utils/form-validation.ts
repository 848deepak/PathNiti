...
export class FormValidator {
  ...
  static validateContactPerson(value: string): ValidationResult {
    ...
    if (sanitized.trim().length < 2) {
      return { isValid: false, error: "Contact person name must be at least 2 characters" }
    }
    
    if (!/^[a-zA-Z\s'-]+$/.test(sanitized.trim())) {
      return { isValid: false, error: "Contact person name can only contain letters, spaces, hyphens, and apostrophes" }
    }
  ...
  static validateForm(formData: any): FormValidationState {
    const validation: FormValidationState = {
      ...
      collegeId: this.validateCollegeId(formData.collegeId || ''),
      contactPerson: this.validateContactPerson(formData.contactPerson || ''),
      designation: this.validateDesignation(formData.designation || ''),
      overall: { isValid: true }
    }

    // Check overall form validity
    const hasErrors = Object.values(validation).some(field => 
      field !== validation.overall && !field.isValid
    )

    validation.overall = {
      isValid: !hasErrors,
      error: hasErrors ? "Please fix the errors above before submitting" : undefined
    }
   ...
   */
  static getFieldError(validation: FormValidationState, fieldName: keyof FormValidationState): string | undefined {
   ...
   */
  static getFieldWarnings(validation: FormValidationState, fieldName: keyof FormValidationState): string[] {
   ...
   */
  static isFieldValid(validation: FormValidationState, fieldName: keyof FormValidationState): boolean {
   ...
   */
  static getSanitizedValue(validation: ValidationResult, originalValue: string): string {
...
export function validateCollegeRegistration(data: CollegeRegistrationData): ComprehensiveValidationResult {
  ...
  // Validate location
  if (!data.location || !data.location.state || data.location.state.trim().length === 0) {
  ...
  if (data.established_year) {
    ...
    if (data.established_year < 1850 || data.established_year > currentYear) {
      errors.established_year = 'Established year must be between 1850 and current year'
    }
  }
...
export function validateStudentApplication(data: StudentApplicationData): ComprehensiveValidationResult {
  const errors: ValidationErrors = {}
...
export function validateCourseData(data: CourseData): ComprehensiveValidationResult {
  const errors: ValidationErrors = {}
...
export function validateNoticeData(data: NoticeData): ComprehensiveValidationResult {
  const errors: ValidationErrors = {}
