...
export class RateLimiter {
   ...
   */
  static createUserBasedLimiter(config: Omit<RateLimitConfig, 'keyGenerator'>): RateLimiter {
    return new RateLimiter({
      ...config,
...
export class BurstProtectedRateLimiter extends RateLimiter {
  ...
  async checkLimit(request: NextRequest): Promise<RateLimitResult> {
    ...
    const burstKey = this.config.keyGenerator(request) + ':burst'
    const burstEntry = memoryStore.increment(burstKey, this.burstConfig.burstWindowMs)
