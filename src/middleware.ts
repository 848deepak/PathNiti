...
class RouteChecker {
   ...
   */
  private static matchesRoutePattern(pathname: string, routes: readonly string[]): boolean {
  ...
  static requiresAuth(pathname: string): boolean {
    return this.isProtectedRoute(pathname) || 
  ...
  static requiresRoleCheck(pathname: string): boolean {
    return this.isAdminRoute(pathname) || this.isCollegeRoute(pathname) || this.isStudentRoute(pathname)
  }
}
...
class RedirectHandler {
   ...
   */
  static toUnauthorized(request: NextRequest, requiredRole?: string): NextResponse {
    const homeUrl = new URL('/', request.url)
...
class SessionValidator {
   ...
   */
  static async validateSession(supabase: any, retries = SESSION_CONFIG.maxRetries) {
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (error) {
          ...
          console.warn(`Session validation attempt ${attempt + 1} failed:`, error.message)
        ...
        if (session) {
          ...
          if (isExpired) {
            ...
            return { session: null, error: new Error('Session expired'), isValid: false }
          }
          
          // Validate that session has required fields
          if (!session.user?.id) {
            console.warn('Session missing user ID')
            return { session: null, error: new Error('Invalid session'), isValid: false }
          }
        }
    ...
    return { session: null, error: new Error('Max retries exceeded'), isValid: false }
  }
...
class ProfileValidator {
   ...
   */
  static async validateProfile(supabase: any, userId: string) {
    try {
      ...
      return { profile, error: null, needsProfileCreation: false, needsProfileCompletion: false }
    } catch (err) {
   ...
   */
  static hasRequiredRole(profile: any, pathname: string): boolean {
...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    const { session, error: sessionError, isValid } = await SessionValidator.validateSession(supabase)
    ...
    if (sessionError && !isValid) {
      ...
      if (RouteChecker.requiresAuth(pathname) && 
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Middleware] Redirecting to login due to network error on protected route: ${pathname}`)
        }
      ...
      if (RouteChecker.requiresAuth(pathname)) {
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Middleware] Allowing request to proceed despite session error - client will handle auth`)
        }
        return response
      }
    ...
    if (RouteChecker.isAuthOnlyRoute(pathname)) {
      if (session) {
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Middleware] Redirecting authenticated user from auth-only route: ${pathname}`)
        }
    ...
    if (!session) {
      if (RouteChecker.requiresAuth(pathname)) {
        ...
        const hasValidAuthCookies = authCookies.some(cookie => 
          cookie.value && cookie.value.length > 10 && !cookie.value.includes('null')
        )
        
        if (hasValidAuthCookies) {
          if (process.env.NODE_ENV === 'development') {
            console.log(`[Middleware] Valid auth cookies present but no session, letting client attempt refresh: ${pathname}`)
          }
          return response
        }
        
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Middleware] No valid session or auth cookies, redirecting to login: ${pathname}`)
        }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[Middleware] User authenticated, checking role requirements for: ${pathname}`)
    }
    ...
    if (RouteChecker.requiresRoleCheck(pathname)) {
      const { profile, error: profileError, needsProfileCreation, needsProfileCompletion } = 
        await ProfileValidator.validateProfile(supabase, session.user.id)
      ...
      if (needsProfileCreation) {
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Middleware] Redirecting to complete profile - creation needed`)
        }
      ...
      if (needsProfileCompletion) {
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Middleware] Redirecting to complete profile - completion needed`)
        }
        return RedirectHandler.toCompleteProfile(request)
      }
      ...
      if (profileError && !profile) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Middleware] Allowing request to proceed despite profile error - client will handle auth`)
        }
        return response
      }
      ...
      if (profile && !ProfileValidator.hasRequiredRole(profile, pathname)) {
        ...
        else if (RouteChecker.isCollegeRoute(pathname)) requiredRole = 'college'
        else if (RouteChecker.isStudentRoute(pathname)) requiredRole = 'student'
        
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Middleware] User role '${profile.role}' insufficient for route requiring '${requiredRole}'`)
        }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[Middleware] Allowing request to proceed despite error - client will handle auth`)
    }
    return response
  }
}
...
function shouldSkipMiddleware(pathname: string): boolean {
  ...
  // Check patterns
  if (skipPatterns.some(pattern => pathname.startsWith(pattern) || pathname.includes(pattern))) {
    return true
  }
